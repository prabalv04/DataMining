...
train_loader = torch.utils.data.DataLoader(
					torchvision.datasets.MNIST("/home/prabalv/DataMining/Assignment1/",
...
def calculate_loss(w, w0, x, y):
	...
	return torch.norm(vec).item()**2

def gradient(w, w0, x, y):
	w1 = 2*torch.matmul(torch.matmul(w, x), x.transpose(0,1))
...
fig = plt.figure()
plt.plot(x_axis, training_loss_arr, "b")
fig.suptitle('Training')
plt.ylabel('Loss')
plt.xlabel('Epoch')
fig.savefig('train'+str(step_size)+".jpg")

fig = plt.figure()
plt.plot(x_axis, test_loss_arr, "r")
fig.suptitle('Test')
plt.ylabel('Loss')
plt.xlabel('Epoch')
fig.savefig('test'+str(step_size)+".jpg")
